...
extension ZegoCallManagerExtension on ZegoCallManager {
  ...
  void onReceiveCallUserAccepted(ZIMCallUserInfo userInfo) {
    ...
    // Inviter is always considered accepted — ZIM never sends 'accepted' state for the inviter,
    // so we must not rely on finding the inviter in callUserList to set meHasAccepted.
    final meIsInviter = currentCallData!.inviter.userID == localUser?.userID;
    var moreThanOneAcceptedExceptMe = false;
    var meHasAccepted = meIsInviter;
    debugPrint('onReceiveCallUserAccepted: meIsInviter=$meIsInviter localUser=${localUser?.userID} inviter=${currentCallData!.inviter.userID} callUserList.length=${currentCallData!.callUserList.length}');
    for (final callUser in currentCallData!.callUserList) {
      if (callUser.userID == localUser?.userID) {
        meHasAccepted = meHasAccepted || callUser.hasAccepted.value;
      } else {
    ...
    debugPrint('onReceiveCallUserAccepted: meHasAccepted=$meHasAccepted moreThanOneAcceptedExceptMe=$moreThanOneAcceptedExceptMe isCallStart=$isCallStart isGroupCall=${currentCallData!.isGroupCall}');
  ...
  void onReceiveCallUserQuit(String requestID, ZIMCallUserInfo userInfo) {
    if (currentCallData != null) {
      ...
      final meIsInviter = currentCallData!.inviter.userID == localUser?.userID;
      if (selfCallUser != null && (selfCallUser.hasAccepted.value || meIsInviter)) {
        var moreThanOneAcceptedExceptMe = false;
  ...
  void checkIfPKEnd(String requestID, ZegoSDKUser currentUser) {
    ...
    if (selfCallUser != null) {
      if (selfCallUser.hasAccepted.value || meIsInviter) {
        var hasWaitingUser = false;
        for (final callUser in currentCallData!.callUserList) {
          if (callUser.userID != localUser?.userID) {
            // except self
