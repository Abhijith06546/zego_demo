...
class ZegoCallManager implements ZegoCallManagerInterface {
  ...
  Future<ZIMCallInvitationSentResult> sendUserRequest(
    ...
    if (successUsers.isNotEmpty) {
      // Include the local user (inviter) in callUserList so CallContainer can render them.
      // ZIM never sends an 'accepted' state event for the inviter themselves, so they must
      // be seeded here with accepted=true.
      final localCallUser = CallUserInfo(userID: localUser?.userID ?? '')
        ..sdkUserNoti.value = localUser
        ..updateCallUserState(ZIMCallUserState.accepted);
      currentCallData!
        ..callID = result.callID
        ..inviter = localCallUser
        ..callType = type == ZegoCallType.video ? VIDEO_Call : VOICE_Call
        ..callUserList = [localCallUser];
    } else {
      clearCallData();
    }
    return result;
